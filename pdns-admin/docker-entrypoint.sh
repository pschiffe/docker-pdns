...
if [[ "${PDNS_ADMIN_SQLA_DB_TYPE}" == 'mysql' ]]; then
    ...
    SQL_COMMAND="mariadb -h ${PDNS_ADMIN_SQLA_DB_HOST} -P ${PDNS_ADMIN_SQLA_DB_PORT} -u ${PDNS_ADMIN_SQLA_DB_USER} ${MYSQL_CLIENT_EXTRA_PARAMS} -p${PDNS_ADMIN_SQLA_DB_PASSWORD} -e"
